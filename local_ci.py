...
def main():
    ...
    group.add_argument("--full", action="store_true",
                       help="Run full CI suite (like GitHub ci.yml)")
    group.add_argument("--pre-commit", action="store_true", 
                       help="Run pre-commit checks only")
    ...
    if args.format:
        ...
        cmd = [sys.executable, str(full_ci_script), 
               "--skip-coverage", "--skip-integration", "--skip-static"]
        
    elif args.static:
        # Run only static analysis
        cmd = [sys.executable, str(full_ci_script),
               "--skip-hooks", "--skip-coverage", "--skip-integration"]
