...
def run_command(
    cmd: List[str], description: str, cwd: Path = None, timeout: int = 300
) -> Tuple[bool, str, str]:
    """
...
def run_static_analysis(missing_tools: List[str]):
    ...
    if "mypy" not in missing_tools:
        success, _, _ = run_command(
            ["mypy", "pure3270/", "--config-file=mypy.ini"], "MyPy type checking"
        )
    ...
    if "bandit" not in missing_tools:
        success, _, _ = run_command(
            ["bandit", "-c", ".bandit", "-r", "pure3270/"], "Bandit security analysis"
        )
        all_passed = all_passed and success
    else:
    ...
    if "flake8" not in missing_tools:
        success, _, _ = run_command(["flake8", "pure3270/"], "Flake8 style checking")
        all_passed = all_passed and success
    else:
...
def run_pre_commit_hooks(missing_tools: List[str]):
    ...
    if "pre-commit" in missing_tools:
        ...
        if "black" not in missing_tools:
            success, _, _ = run_command(
                ["black", "--check", "pure3270/"], "Black formatting check"
            )
            all_passed = all_passed and success
        ...
        if "isort" not in missing_tools:
            success, _, _ = run_command(
                ["isort", "--check-only", "pure3270/"], "Isort import sorting check"
            )
            all_passed = all_passed and success
...
def run_coverage_tests(missing_tools: List[str]):
    ...
    try:
        subprocess.run(
            ["python", "-c", "import pytest_cov"], capture_output=True, check=True
        )
...
def main():
    ...
    parser.add_argument(
        "--skip-coverage", action="store_true", help="Skip coverage reporting"
    )
    parser.add_argument(
