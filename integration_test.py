...
print(
    "[INTEGRATION DEBUG] Imported QUERY_REPLY_SF, QUERY_REPLY_CHARACTERISTICS from protocol.data_stream"
)
...
print(
    "[INTEGRATION DEBUG] All imports completed, about to define test_basic_functionality"
)
print(
    "[INTEGRATION DEBUG] All imports completed, about to define test_basic_functionality"
)
...
class TN3270ENegotiatingMockServer(MockServer):
    ...
    def _maybe_set_negotiation_complete(self):
        if self._negotiated_device_type and self._negotiated_functions:
            if not self.negotiation_complete.is_set():
                print(
                    "Mock Server: TN3270E negotiation fully complete (SNA). Setting negotiation_complete event."
                )
    ...
    async def handle_client(self, reader, writer):
        ...
        try:
            ...
            while True:
                print(
                    f"[MOCK SERVER DEBUG] Entering read loop iteration, waiting for data..."
                )
                try:
                    data = await asyncio.wait_for(reader.read(1024), timeout=10.0)
                    print(
                        f"[MOCK SERVER DEBUG] Received {len(data)} bytes: {data.hex()}"
                    )
                except asyncio.TimeoutError:
                        ...
                        if i + 1 < len(data):
                            ...
                            if command == DO:
                                ...
                                if j + 1 < len(data) and data[j + 1] == SE:
                                    ...
                                    sub_data = data[i + 3 : j]
                                    print(
                                        f"[MOCK SERVER DEBUG] Processing SB: sub_option=0x{sub_option:02x}, sub_data={sub_data.hex()}"
                                    )
                                    if sub_option == TELOPT_TN3270E:
                                        if len(sub_data) >= 2:
                                            ...
                                            print(
                                                f"[MOCK SERVER DEBUG] TN3270E sub: type=0x{tn3270e_type:02x}, subtype=0x{tn3270e_subtype:02x}"
                                            )
                                            if (
                                                tn3270e_type == TN3270E_DEVICE_TYPE
                                                and tn3270e_subtype == TN3270E_SEND
                                            ):
                                                ...
                                                print(
                                                    "[MOCK SERVER DEBUG] Sent DEVICE-TYPE IS response."
                                                )
                                                self._maybe_set_negotiation_complete()
                                            elif (
                                                tn3270e_type == TN3270E_FUNCTIONS
                                                and tn3270e_subtype == TN3270E_SEND
                                            ):
                                                functions_response = bytes(
                                                    [
                                                        TN3270E_BIND_IMAGE
                                                        | TN3270E_RESPONSES
                                                    ]
                                                ...
                                                print(
                                                    "[MOCK SERVER DEBUG] Sent FUNCTIONS IS response."
                                                )
                                                self._maybe_set_negotiation_complete()
                                            else:
                                                print(
                                                    f"[MOCK SERVER DEBUG] Unhandled TN3270E sub: type=0x{tn3270e_type:02x}, subtype=0x{tn3270e_subtype:02x}"
                                                )
                                        else:
                                            print(
                                                f"[MOCK SERVER DEBUG] Incomplete TN3270E sub_data: {sub_data.hex()}"
                                            )
                                    else:
                                        print(
                                            f"[MOCK SERVER DEBUG] Unhandled SB sub_option: 0x{sub_option:02x}"
                                        )
                                    i = j + 2
                                else:
                                    print(
                                        f"[MOCK SERVER DEBUG] Incomplete SB, no SE found."
                                    )
                                    break
                            else:
                                i += 2
                ...
                if self.negotiation_complete.is_set() and not negotiation_done:
                    print(
                        "[MOCK SERVER DEBUG] TN3270E negotiation complete (base class)."
                    )
                            ...
                            if command == DO:
                                if i + 2 < len(data):
                                    ...
                                    if option == TELOPT_TTYPE:
                                        ...
                                        response.extend(
                                            bytes([IAC, WILL, TELOPT_TERMINAL_LOCATION])
                                        )
                                ...
                                if i + 2 < len(data):
                                    ...
                                    if option == TELOPT_TTYPE:
                                        ...
                                        response.extend(
                                            bytes([IAC, DO, TELOPT_TERMINAL_LOCATION])
                                        )
                                ...
                                if j + 1 < len(data) and data[j + 1] == SE:
                                    ...
                                    if sub_option == TELOPT_TERMINAL_LOCATION:
                                        if (
                                            len(sub_data) >= 1
                                            and sub_data[0] == TN3270E_IS
                                        ):
                                            self.received_lu_name = sub_data[1:].decode(
                                                "ascii", errors="ignore"
                                            )
                                            print(
                                                f"Mock Server: Received LU Name: {self.received_lu_name}"
                                            )
            ...
            while True:
                ...
                while i < len(data):
                    if data[i] == IAC:
                        ...
                        data[i] == SOH
                    ):  # Check for SOH messages directly in stream (for printer status)
                        if i + 1 < len(data):
                            self.received_soh_status = data[i + 1]
                            print(
                                f"Mock Server: Received SOH message with status: 0x{self.received_soh_status:02x}"
                            )
                        ...
                        data[i] == STRUCTURED_FIELD
                    ):  # Check for Structured Field (for printer status SF)
                        if i + 3 < len(data):  # SF_ID + 2-byte Length + SF_Type
                            ...
                            if sf_type == PRINTER_STATUS_SF_TYPE:
                                if i + 4 < len(data):
                                    ...
                                    print(
                                        f"Mock Server: Received Printer Status SF with code: 0x{self.received_printer_status_sf_code:02x}"
                                    )
                ...
                if self.negotiation_complete.is_set() and not negotiation_done:
                    print(
                        "Mock Server: Negotiation complete. Sending printer status messages."
                    )
                    # Send SOH messages
                    ...
                    await writer.drain()
                    print(
                        f"Mock Server: Sent SOH device end: {soh_device_end_msg.hex()}"
                    )

                    soh_intervention_required_msg = bytes(
                        [SOH, SOH_INTERVENTION_REQUIRED]
                    )
                    writer.write(soh_intervention_required_msg)
                    await writer.drain()
                    print(
                        f"Mock Server: Sent SOH intervention required: {soh_intervention_required_msg.hex()}"
                    )

                    # Send Printer Status Structured Fields
                    # SF format: 0x3C (SF_ID), Length (2 bytes), SF_Type (1 byte), Data
                    printer_sf_success_data = bytes(
                        [PRINTER_STATUS_SF_TYPE, 0x00]
                    )  # Type + status
                    printer_sf_success = (
                        bytes([STRUCTURED_FIELD])
                        + (len(printer_sf_success_data) + 2).to_bytes(2, "big")
                        + printer_sf_success_data
                    )
                    writer.write(printer_sf_success)
                    await writer.drain()
                    print(
                        f"Mock Server: Sent Printer Status SF success: {printer_sf_success.hex()}"
                    )
    ...
    async def handle_client(self, reader, writer):
        ...
        try:
            ...
            while True:
                                        ...
                                        if len(sub_data) >= 2:
                                            ...
                                            if (
                                                ...
                                                if not self._negotiated_device_type:
                                                    ...
                                                    await writer.drain()
                                                    print(
                                                        "Mock Server: Sent DEVICE-TYPE IS response immediately"
                                                    )
                                                    self._negotiated_device_type = True
                                                    self._maybe_set_negotiation_complete()
                                            elif (
                                                ...
                                                if not self._negotiated_functions:
                                                    ...
                                                    await writer.drain()
                                                    print(
                                                        f"Mock Server: Sent FUNCTIONS IS response immediately"
                                                    )
                                                    self._negotiated_functions = True
                                                    self._maybe_set_negotiation_complete()
                                    i = j + 2
                ...
                if self.negotiation_complete.is_set() and not negotiation_done:
                    print(
                        "Mock Server: TN3270E negotiation complete (BindImageMockServer). Sending BIND-IMAGE."
                    )
                    ...
                    await writer.drain()
                    print(
                        f"Mock Server: Sent BIND-IMAGE after negotiation: {bind_message.hex()}"
                    )
                    negotiation_done = True
        except Exception as e:
    ...
    async def handle_client(self, reader, writer):
        ...
        try:
            ...
            while True:
                ...
                print(
                    f"[MOCK SERVER DEBUG] SNAAware: Entering read loop iteration, waiting for data..."
                )
                try:
                    data = await asyncio.wait_for(reader.read(1024), timeout=30.0)
                    print(
                        f"[MOCK SERVER DEBUG] SNAAware: Data received at {time.time()}: {len(data)} bytes"
                    )
                    print(
                        f"[MOCK SERVER DEBUG] SNAAware: Received {len(data)} bytes: {data.hex()}"
                    )
                except asyncio.TimeoutError:
                    print(
                        f"[MOCK SERVER DEBUG] SNAAware: Read timeout, continuing loop."
                    )
                    continue
                if not data:
                    print(
                        f"[MOCK SERVER DEBUG] SNAAware: No data received, breaking loop."
                    )
                    break

                print(
                    f"[MOCK SERVER DEBUG] SNAAware: Client connection active, writer.is_closing(): {writer.is_closing()}"
                )
                print(
                    f"[MOCK SERVER DEBUG] SNAAware: Starting IAC parsing on received data..."
                )
                ...
                while i < len(data):
                    if data[i] == IAC:
                        print(
                            f"[MOCK SERVER DEBUG] SNAAware: Found IAC at position {i}, next bytes: {data[i:i+3].hex() if i+3 <= len(data) else data[i:].hex()}"
                        )
                        if i + 1 < len(data):
                            command = data[i + 1]
                            if command == DO:
                                if i + 2 < len(data):
                                    ...
                                    if option == TELOPT_TTYPE:
                                        print(
                                            "Mock Server: Responding WILL TTYPE to DO TTYPE"
                                        )
                                        response.extend(
                                            bytes([IAC, WILL, TELOPT_TTYPE])
                                        )
                                        ...
                                        print(
                                            "Mock Server: Responding WILL BINARY to DO BINARY"
                                        )
                                        response.extend(
                                            bytes([IAC, WILL, TELOPT_BINARY])
                                        )
                                        ...
                                        print(
                                            "Mock Server: Responding WILL EOR to DO EOR"
                                        )
                                        response.extend(bytes([IAC, WILL, TELOPT_EOR]))
                                    elif option == TELOPT_TN3270E:
                                        print(
                                            "Mock Server: Responding WILL TN3270E to DO TN3270E"
                                        )
                                        response.extend(
                                            bytes([IAC, WILL, TELOPT_TN3270E])
                                        )
                                        ...
                                        print(
                                            f"Mock Server: Responding WONT {option} to DO {option}"
                                        )
                                        response.extend(bytes([IAC, WONT, option]))
                                    i += 3
                                else:
                                ...
                                if i + 2 < len(data):
                                    ...
                                    if option == TELOPT_TTYPE:
                                        ...
                                        if not sent_telnet_options:
                                            response.extend(
                                                bytes([IAC, DO, TELOPT_BINARY])
                                            )
                                            response.extend(
                                                bytes([IAC, WILL, TELOPT_BINARY])
                                            )
                                            response.extend(
                                                bytes([IAC, DO, TELOPT_EOR])
                                            )
                                            response.extend(
                                                bytes([IAC, WILL, TELOPT_EOR])
                                            )
                                            response.extend(
                                                bytes([IAC, DO, TELOPT_TN3270E])
                                            )
                                            response.extend(
                                                bytes([IAC, WILL, TELOPT_TN3270E])
                                            )
                                        ...
                                        print(
                                            "Mock Server: Responding SB TN3270E DEVICE-TYPE SEND"
                                        )
                                        response.extend(
                                            bytes(
                                                [
                                ...
                                if j + 1 < len(data) and data[j + 1] == SE:
                                    ...
                                    sub_data = data[i + 3 : j]
                                    print(
                                        f"[MOCK SERVER DEBUG] SNAAware: Processing SB: sub_option=0x{sub_option:02x}, sub_data={sub_data.hex()}"
                                    )
                                    if sub_option == TELOPT_TN3270E:
                                        if len(sub_data) >= 2:
                                            tn3270e_type = sub_data[0]
                                            tn3270e_subtype = sub_data[1]
                                            print(
                                                f"[MOCK SERVER DEBUG] SNAAware: TN3270E sub: type=0x{tn3270e_type:02x}, subtype=0x{tn3270e_subtype:02x}"
                                            )
                                            if (
                                                tn3270e_type == TN3270E_DEVICE_TYPE
                                                and tn3270e_subtype == TN3270E_SEND
                                                ...
                                                print(
                                                    "[MOCK SERVER DEBUG] SNAAware: Received DEVICE-TYPE SEND subnegotiation."
                                                )
                                                if not self._negotiated_device_type:
                                                    from pure3270.protocol.utils import \
                                                        TN3270E_IBM_DYNAMIC
                                                    ...
                                                    print(
                                                        "[MOCK SERVER DEBUG] SNAAware: Sending DEVICE-TYPE IS response (SNA)..."
                                                    )
                                                    writer.write(
                                                        bytes(
                                                            [
                                                    ...
                                                    self._maybe_set_negotiation_complete()
                                                    print(
                                                        "[MOCK SERVER DEBUG] SNAAware: Sent DEVICE-TYPE IS response (SNA). Negotiation forced complete for test."
                                                    )
                                                    print(
                                                        "[MOCK SERVER DEBUG] SNAAware: Both DEVICE-TYPE and FUNCTIONS negotiated. Setting negotiation_complete."
                                                    )
                                            elif (
                                                tn3270e_type == TN3270E_FUNCTIONS
                                                and tn3270e_subtype == TN3270E_SEND
                                                ...
                                                print(
                                                    "[MOCK SERVER DEBUG] SNAAware: Received FUNCTIONS SEND subnegotiation."
                                                )
                                                if not self._negotiated_functions:
                                                    functions_response = bytes(
                                                        [
                                                    ...
                                                    print(
                                                        "[MOCK SERVER DEBUG] SNAAware: Sending FUNCTIONS IS response (SNA)..."
                                                    )
                                                    writer.write(
                                                        bytes(
                                                            [
                                                    ...
                                                    self._negotiated_functions = True
                                                    print(
                                                        "[MOCK SERVER DEBUG] SNAAware: Sent FUNCTIONS IS response (SNA)."
                                                    )
                                                ...
                                                print(
                                                    f"[MOCK SERVER DEBUG] SNAAware: Unhandled TN3270E sub: type=0x{tn3270e_type:02x}, subtype=0x{tn3270e_subtype:02x}"
                                                )
                                        else:
                                            print(
                                                f"[MOCK SERVER DEBUG] SNAAware: Incomplete TN3270E sub_data: {sub_data.hex()}"
                                            )
                                        ...
                                        if len(sub_data) > 1 and sub_data[0] == 0:  # IS
                                            ...
                                            print(
                                                f"[MOCK SERVER DEBUG] SNAAware: Received TTYPE IS with types: {[t.decode(errors='ignore') for t in ttypes]}"
                                            )
                                            if b"IBM-DYNAMIC" in ttypes:
                                                print(
                                                    "[MOCK SERVER DEBUG] SNAAware: IBM-DYNAMIC found, proceeding with negotiation."
                                                )
                                        ...
                                        if (
                                            len(sub_data) >= 1 and sub_data[0] == 1
                                        ):  # SEND
                                            if not hasattr(self, "_ttype_index"):
                                                self._ttype_index = 0
                                            terminal_type = ttype_list[
                                                self._ttype_index
                                            ]
                                            writer.write(
                                                bytes(
                                            ...
                                            await writer.drain()
                                            print(
                                                f"[MOCK SERVER DEBUG] SNAAware: Sent TTYPE IS response (SNA): {terminal_type.decode()}"
                                            )
                                            self._ttype_index += 1
                                        ...
                                        print(
                                            f"[MOCK SERVER DEBUG] SNAAware: Unhandled SB sub_option: 0x{sub_option:02x}"
                                        )
                                    i = j + 2
                                else:
                                    print(
                                        f"[MOCK SERVER DEBUG] SNAAware: Incomplete SB, no SE found."
                                    )
                                    break
                            else:
                                i += 2
                ...
                if response:
                    print(
                        f"[MOCK SERVER DEBUG] SNAAware: Sending response: {response.hex()}"
                    )
                    # Log each byte sent
                    for idx, b in enumerate(response):
                        print(
                            f"[MOCK SERVER DEBUG] SNAAware: Byte sent [{idx}]: 0x{b:02x}"
                        )
                    print(
                        f"[MOCK SERVER DEBUG] SNAAware: Write/drain start at {time.time()}"
                    )
                    writer.write(response)
                    await writer.drain()
                    print(
                        f"[MOCK SERVER DEBUG] SNAAware: Write/drain end at {time.time()}"
                    )
                    print(f"[MOCK SERVER DEBUG] SNAAware: Response drained.")
                else:
                    print(
                        f"[MOCK SERVER DEBUG] SNAAware: No response to send in this iteration."
                    )
                    print(
                        f"[MOCK SERVER DEBUG] SNAAware: No IAC matched, possible non-IAC data or parse error."
                    )
                if self.negotiation_complete.is_set() and not negotiation_done:
                    print(
                        "Mock Server: negotiation_complete event is set. About to send BIND-IMAGE."
                    )
                    ...
                    total_length = (
                        2 + 1 + len(bind_data)
                    )  # length field (2) + type (1) + bind_data
                    sf.extend(total_length.to_bytes(2, "big"))
                    sf.append(0x03)
                    sf.extend(bind_data)
                    bind_header = TN3270EHeader(
            ...
            print(
                f"[MOCK SERVER DEBUG] SNAAware: Closing client connection, writer.is_closing(): {writer.is_closing()}"
            )
            if writer in self.clients:
                self.clients.remove(writer)
            writer.close()
...
async def test_sna_response_handling(port, mock_server):
    ...
    try:
        ...
        await asyncio.wait_for(
            getattr(mock_server, "negotiation_complete", asyncio.Event()).wait(),
            timeout=30,
        )
        ...
        if (
            hasattr(session.screen_buffer, "rows")
            and session.screen_buffer.rows == mock_server.rows
            and session.screen_buffer.cols == mock_server.cols
        ):
            print(
                f"   ✓ Screen dimensions updated by BIND-IMAGE: {session.screen_buffer.rows}x{session.screen_buffer.cols}"
            )
        else:
            print(
                f"   ⚠ Screen dimensions may not be updated: expected {mock_server.rows}x{mock_server.cols}, got {getattr(session.screen_buffer, 'rows', 'N/A')}x{getattr(session.screen_buffer, 'cols', 'N/A')}, but continuing"
            )
        ...
        parser = (
            session.handler.parser
            if hasattr(session, "handler") and session.handler
            else None
        )
        if parser:
            try:
                ...
                if (
                    sna_response.is_positive()
                    and sna_response.sense_code == SNA_SENSE_CODE_SUCCESS
                ):
                    print("   ✓ SNA response parsing successful")
                else:
                    print(
                        "   ⚠ SNA response parsing returned unexpected result, but continuing"
                    )
            except Exception as e:
        ...
        unknown_sf = bytes(
            [0x3C, 0x00, 0x05, 0xFF, 0xAA, 0xBB]
        )  # SF with unknown type 0xFF
        try:
        ...
        try:
            ...
            if hasattr(parser.screen, "get_position"):
                ...
                print(
                    f"   ✓ RA order handling: position now at ({current_row}, {current_col})"
                )
            else:
...
async def test_printer_status(port, mock_server):
    ...
    try:
        ...
        try:
            ...
            print(
                "   ⚠ Client SOH response not received within timeout, but continuing"
            )

        try:
            await asyncio.wait_for(
                mock_server.client_printer_status_sf_received.wait(), timeout=10.0
            )
            print("   ✓ Client Printer Status SF response received")
        except asyncio.TimeoutError:
            print(
                "   ⚠ Client Printer Status SF response not received within timeout, but continuing"
            )

        # Optional: Verify session state (e.g., no errors in handler)
        # Use the correct attribute name
        is_connected = getattr(
            session, "is_connected", getattr(session, "connected", None)
        )
        ...
        if is_connected:
            ...
            print(
                "   ⚠ Session disconnected after printer status exchange, but continuing test"
            )
...
async def main():
    ...
    timeout_success, timeout_result = run_with_limits_sync(
        test_timeout_verification, int_time, int_mem
    )
    if timeout_success:
    ...
    basic_success, basic_result = run_with_limits_sync(
        test_basic_functionality, int_time, int_mem
    )
